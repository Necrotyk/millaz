...
func handleAsk(bot *Bot, sender, channel, query string, provider IRCProvider) {
	...
	reply, err := CallGeminiText(key, "gemini-2.5-flash-lite", persona, maskedSender, maskedQuery)
	if err != nil {
...
func handleRecap(bot *Bot, channel string, provider IRCProvider) {
	...
	summary, err := CallGeminiText(key, "gemini-2.5-flash-lite", persona, "system", prompt)
	if err != nil {
